    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">Manage brand tone pillars per product</p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 text-left">
                    ...
                    <th className="px-4 py-3 font-medium text-slate-600">Do&apos;s</th>
                    <th className="px-4 py-3 font-medium text-slate-600">Don&apos;ts</th>
                ...
                <tbody>
                  {filtered.map((p) => (
                    <tr key={p.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-slate-500">{productName(p.product_id)}</td>
                      <td className="max-w-xs truncate px-4 py-3 text-slate-500">{p.description ?? '—'}</td>
                      <td className="max-w-xs truncate px-4 py-3 text-slate-500">{p.do_example ?? '—'}</td>
                      <td className="max-w-xs truncate px-4 py-3 text-slate-500">{p.dont_example ?? '—'}</td>
