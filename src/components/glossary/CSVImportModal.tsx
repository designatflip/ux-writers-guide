    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-neutral-200 px-6 py-4">
          <div>
            <h2 className="text-base font-semibold text-neutral-900">Import from CSV</h2>
        ...
        <div className="flex-1 overflow-auto px-6 py-5 space-y-4">
          {/* File picker */}
          <div
            className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-neutral-200 py-8 hover:border-flip-orange-300 hover:bg-flip-orange-100/30 transition-colors"
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-neutral-200 bg-neutral-50 text-left">
                      <th className="px-3 py-2 font-medium text-neutral-600">Term (EN)</th>
                      <th className="px-3 py-2 font-medium text-neutral-600">Term (ID)</th>
                      <th className="px-3 py-2 font-medium text-neutral-600">Definition</th>
                      <th className="px-3 py-2 font-medium text-neutral-600">Avoid</th>
                  ...
                  <tbody>
                    {rows.map((row, i) => (
                      <tr
                        key={i}
                        className={`border-b border-neutral-50 last:border-0 ${row.error ? 'bg-crimson-100' : ''}`}
                      >
                        <td className="px-3 py-2 font-medium text-neutral-900 max-w-[130px] truncate">
                          {row.error ? (
                            <span className="text-crimson italic">{row.error}</span>
                          ) : row.term}
                        </td>
                        <td className="px-3 py-2 text-neutral-600 max-w-[120px] truncate">{row.term_bahasa || '—'}</td>
                        <td className="px-3 py-2 text-neutral-600 max-w-[200px] truncate">{row.definition}</td>
                        <td className="px-3 py-2 text-neutral-600 max-w-[120px] truncate">{row.avoid || '—'}</td>
